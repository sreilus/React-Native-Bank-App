...
class Havale extends React.Component {
    ...
    makeHavale = async (balance, recieverAccNo, statement) => {
        ...
        if (this.state.tcNumber !== null) {
            ...
            let url = 'https://rugratswebapi.azurewebsites.net/api/moneytransfers/havale';
            console.log('url: ' + url)
            this.setState({
                isLoading: true
            })
            await fetch(url, {
                ...
                if (deger == "1") {
                    ...
                    Alert.alert("Alıcıya ait aktif hesap bulunamadı!");
                }
                else if (deger == "6") {
                    Alert.alert("Aynı Hesaba Para Gönderemezsiniz!");
                }
                else {
                    console.log("kapatt : " + deger)
                    alert("Para Gönderme İşlemi Başarısız Oldu!");
                }
               // this.setState({ defaultAnimationModal: false });
                // console.log("finally " + this.state.accounts[0].accountNo)
            })
        ...
        if (this.state.isLoading === false) {
            return (
                <View style={styles.container}>
                    <Text style={{ marginTop: 5, marginBottom: 5, fontSize: 18, marginLeft: Dimensions.get("window").width * 0.38 }}>Havale</Text>

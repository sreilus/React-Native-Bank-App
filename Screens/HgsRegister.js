...
export default class HgsRegister extends React.Component {
    ...
    registerHgs = async (balance) => {
        if (this.state.tcNumber !== null) {
            console.log('tc: ' + this.state.tcNumber)
            let url = 'https://rugratswebapi.azurewebsites.net/api/hgs';
            console.log('url: ' + url)
            this.setState({
                isLoading: true
            })
            await fetch(url, {
                ...
                if (deger > "1") {
                    Alert.alert("Hgs Başarıyla Açıldı! Yeni HgsNo: " + deger);
                    this.listAccounts();
                }
                else if (deger == "0") {
                    Alert.alert("CustomerId boş bırakılamaz!");
                }
                else if (deger == "3") {
                    Alert.alert("CGeçersiz bir para miktarı girdiniz!");
                }
                else if (deger == "5") {
                    ...
                    console.log("kapatt : " + deger)
                    alert("Hgs Açma İşlemi Başarısız Oldu!");
                }
                // console.log("finally " + this.state.accounts[0].accountNo)
            })
                .catch((error) => {
        ...
        if (this.state.isLoading === false) {
            return (
                <View style={styles.container}>
                    <Text style={{ marginTop: 5, marginBottom: 5, fontSize: 18, marginLeft: Dimensions.get("window").width * 0.38 }}>Hgs Hesabı Aç</Text>
                            ...
                            <TouchableWithoutFeedback style={{
                                ...
                                <View style={styles.view}>
                                    ...
                                    <View style={styles.item}>
                                        <Button title="Hgs Hesabı Aç" onPress={() => this.setState({ isDialogVisible: true, selectedAccountNo: item.accountNo })}></Button>
                                    </View>
                                </View>
                    ...
                    <DialogInput isDialogVisible={this.state.isDialogVisible}
                        ...
                        submitInput={(inputText) => { this.registerHgs(inputText) }}
